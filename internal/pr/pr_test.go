...
func TestBranchNameAndFetchRef(t *testing.T) {
	...
	assert.Equal(t, Meta{Number: 51, Type: "head"}.FetchRef(), "refs/pull/51/head")
	assert.Equal(t, Meta{Number: 9, Type: "merge"}.FetchRef(), "refs/pull/9/merge")
}
...
func TestParseRefs(t *testing.T) {
	...
	for name, tt := range cases {
		t.Run(name, func(t *testing.T) {
			assert.EqualArrays(t, ParseRefs(tt.input), tt.expected)
		})
	}
}
