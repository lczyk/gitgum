...
Commands can have a one-letter alias: `gg b` is `gg branch`, `gg p` is `gg push`, `gg s` is `gg status`, `gg t` is `gg tree`, `gg w` is `gg worktree-switch` (as is `gg w+`, and `gg w-` is `gg worktree-switch-`). Aliases are listed in `gg --help` next to their command, and the completions treat them like the full name (nushell excepted: `gg s` runs, but only `gg status` completes its sections).
...
Push the current branch. Picks a remote interactively when the branch has no upstream -- or has one git won't push to, such as a differently-named upstream under `push.default=simple` (typically the PR base) -- and otherwise confirms a push to the existing tracking branch. Name a remote -- `gg push <remote>`, or part of one to seed the picker -- to push there instead, even when the branch has an upstream; the upstream then moves to it.

When the branch pushes to its upstream, push first asks the remote where that branch is now -- one round trip, plus a fetch of just that branch when its tip is new here -- so it compares against fresh remote state rather than a stale tracking ref, then acts on where local sits: strictly ahead pushes as usual; the remote holding commits you lack (a plain push would be rejected as non-fast-forward) is handled without dropping you into a surprise rejection. If a `pull --rebase` would apply cleanly it offers to rebase and push the result; if it would conflict it errors and points you at `gg pull` to integrate manually. A branch that is merely behind is offered a fast-forward instead. Declining any of these -- or a rebase that would conflict -- offers the other remotes rather than stopping, and so does declining a remote you then pick. With none left, a decline says nothing was pushed and a conflict stays an error. A remote that can't be reached (no answer within 15 seconds counts) is handled the same way: the others are offered. If the push after a rebase fails, or you press Ctrl-C part-way, push undoes the rebase and puts any stashed changes back -- unless the remote turns out to have the push already, or can't be asked, in which case it keeps the rebase and prints how to undo it.
...
### `gitgum worktree-switch [N]` (`gg w`, `gg w+`, `gg w-`)
...
- `gg w` / `gg w+` -- cycle to the next number, wrapping from the highest back to 1; from a directory that isn't a numbered worktree it lands on 1
- `gg w-` -- cycle to the previous number, wrapping from 1 back to the highest; from a directory that isn't a numbered worktree it lands on the highest

The long spellings are `worktree-switch+` and `worktree-switch-`. Before the switch, the worktree being left and the one being entered are each printed the way [`gg status head`](#gitgum-status-gg-s) prints them:

```
* (lczyk/)main 87538fa
* (lczyk/)feat/x 1a2b3c4 [ahead 2]
```

The binary itself only prints the chosen path on stdout (a subdirectory of a worktree counts as that worktree) -- those two lines go to stderr; the `cd` happens in the [shell function](#the-cd-wrapper) `gg completion --cd` prints. Nothing is stashed or checked out -- it's a `cd`.
