...
func TestHeadFloat_OptOut(t *testing.T) {
	t.Parallel()
	// Same nodes as TipSinksToBottom, but Opt.NoHeadFloat tells Layout to ignore
	// IsHead: the newer tip reclaims the bottom row. IsHead stays set either way
	// -- marking HEAD is the caller's job, floating it is Layout's.
	nodes := []graph.Node{
		{ID: "base", Label: "base", Parents: nil, Epoch: 1},
		{ID: "x", Label: "x head", Parents: []string{"base"}, Epoch: 5, IsHead: true},
		{ID: "y", Label: "y other", Parents: []string{"base"}, Epoch: 10},
	}
	lines := graph.Render(graph.Layout(nodes, graph.Opt{NoHeadFloat: true}), graph.Style{})
	assert.Equal(t, indexOf(lines, "y other"), len(lines)-1) // newest at bottom, HEAD ignored
	assert.That(t, indexOf(lines, "x head") < indexOf(lines, "y other"), "HEAD no longer pinned to the bottom")
}
